...
/*
▶ 네비게이션
- 일반적으로 AI에게 레벨을 어떻게 돌아다녀야 할지 알려줄 정보를 담고 있을 객체

- 언리얼에서는 1. 네비게이션 / 2.네브메시 / 3.EQS 나뉜다.
쓰고 최적화를 할 생각 해야 된다.
1. 네비게이션
- 게임 오브젝트가 길을 찾을 때 주로 사용하는 객체
- Bake를 전제로 동작하기 때문에 길찾기 치고는 그나마 가볍다.
- 단점은 Bake를 기반으로 동작하기 때문에 맵의 크기 + 오브젝트 + 버텍스에 영향을 많이 받을 수 밖에 없다.

2. 네브메시
- 네비게이션 메시의 줄임말
- 스테이지를 구성하는 오브젝트의 Mesh 정보를 읽어오고 분석하여 추적할 수 있는 영역 / 지나갈 수 없는 영역으로 미리 데이터화 시킨다.

- Bake를 사용하기도 하지만 실시간 연산도 지원을 하기 때문에 최적화를 위해서는 고민이 필요하다.

- 팁을 주자면 맵의 크기에 상관없이 네브메시는 여러개를쓰는게 크게 1가지를 쓰는 것 보다 더 좋다.

3. EQS
-> EQS 쿼리는 언리얼의 인공 지능 시스템
ㄴ 배치된 환경으로 데이터를 수집하고 데이터에 대한 정의를 하여 가장 적합한 선택지를 생성하고 실행하는 시스템
 ㄴ 비헤이비어 트리에서 호출이 되고 테스트 결과에 따른 의사 결정을 내릴때 사용할 수 있다.
  - 구성요소는 크게 2가지로 생성기 + 컨텍스트가 존재한다.
 ㄴ 생성기 : 가중치를 적용할 위치 / 엑터에게 적용하여 생성하는데 사용이 되는 객체.
 ㄴ 컨텍스트 : 생성기의 레퍼런스로 사용이 된다.
-> 기본적으로 맵에 노드를 지정해주어 가중치에 기반해서 질의 하고 찾아가는 시스템. (범위를 지정해 줄 수 있다)
-> 보스한테 EQS를 쓰는게 좋을 것이다. (보스랑 1대1 매치) (몬스터가 많은 곳에선 네브메시)
-> 취업하는데 도움이 되는건 (1, 2), 3번 EQS는 추가적으로 알고 있으면 회사 취업에 도움이 되는 것.

▶ 언리얼 AI
1. Behavior Tree (행동 트리) (근육)
 - 적 캐릭터의 의사결정에 관여한다.
  ㄴ 필요 로직을 여기서 구현 (액션)

2. AI Controller (컨트롤러) (신경)
 - AI 컨트롤러는 캐릭터와 행동 트리를 연결해 주는 객체
  ㄴ 행동 트리에서 만들어진 정보 및 액션을 캐릭터에게 전달해서 액션이 실제 발생하게 만든다.

3. BlackBoard (데이터를 기입할 보드) (뇌)
- 블랙보드는 행동 트리에서 사용되는 모든 데이터를 담고 있는 컨테이너

▷ 행동 트리 분기

ㆍ 분기 노드
1. 셀렉터
- 아래쪽으로 연결된 모든 노드를 실행하는 노드
- 아래쪽으로 연결된 노드들은 자식이라고 하며, 기본적으로 '왼쪽 -> 오른쪽' 으로 실행된다.

★ 이때 실행 된 자식이 성공적으로 실행이 됐다면 그곳에서 실행을 멈추고 다음 노드들을 실행하지 않는다.
ㄴ 셀렉터가 3개의 자식을 가지고 있다. -> 세번째 위치한 노드가 실행이 되고 싶다. -> 앞에 2개의 노드가 실패해야
실행이 된다.

2. 시퀀스
- 모든 자식을 왼쪽 -> 오른쪽 실행
★ 하나의 자식이라도 실패하면 바로 실행이 멈춘다. -> 시퀀스 종료

BehaviorTree 에서
테스크노드
-> Move To -> 허용 가능한 반경 및 콜리전 크기는 될 수 있으면 좀 여유있게 잡는 것이 좋다. -> 캐릭터 덜덜 떨림 방지
-> 필터 클래스 -> 네비 매쉬 지정하는 곳 -> None일시 기본으로
...
- FBX 포맷(오토데스크)이 있어야 엔진에서 지원하는 다양한 기능을 별다른 선행 처리 없이 사용이 가능하다.
ㄴ 이는 각 기능마다 맵핑이 가능한 포맷이라는 얘기가 된다.

- 기본 개념
- 1. 휴머노이드
ㄴ 머리 / 팔 2개 / 다리 2개 / 몸통 => 휴머노이드 객체 => 인간형
ㄴ 인간형 객체들은 기본적으로 "리타겟팅"을 사용할 수 있다. (애니메이션의 재활용)

- 2. 제너릭
ㄴ 휴머노이드를 제외한 나머지 오브젝트들이 대부분 제너릭으로 들어온다.
ㄴ "일반적으로" 리타겟팅을 사용할 순 없지만, 그럼에도 애니메이션을 사용하고 싶을 때 사용한다.
(늑대와 개는 비슷하듯 예외는 있음)

- 3. 레거시
ㄴ 휴머노이드 약간 + 제너릭 약간 = 레거시 => 주로 엔진 버전업이 되면서 남겨진 기법
ㄴ 대체적으로 코드 기반이 많다.

▷ 페르소나(애니메이션 블루프린트) vs 메카님

- 언리얼은 페르소나 / 유니티는 메카님
ㄴ 애니메이션 상태 머신 시스템
ㄴ 상태 머신 시스템에 공통점은 애니메이션 재생을 관리하고
   트랜지션 로직을 사용해 애니메이션 스테이트를 관리한다.

- 페르소나와 메카님은 재생산성과 확장성 및 이식성이 매우 좋다.
ㄴ 모델의 본 구조만 일치한다면 다른 애니메이션이나 모션 애니메이션을 적용할 수 있기 때문에

- 블랜드스페이스 / 블랜드트리(유니티) 분기가 되는 애니메이션 관리 시스템을 지원한다. 블렌딩(애니메이션 혼합)
ㄴ 두 엔진 모두 해당
- 블랜드스페이스 (블랜딩, 관리(관리는 코드로 관리하는게 더 편리할 수 있다))

- 두 엔진의 애니메이션 상태 머신은 기본적으로 FSM이라고 불리는 기법을 사용한다.
ㄴ 상황에 따라 움직임을 설계할 수 이는 인터페이스를 제공하는 듯 의미.

- 언리얼 애니메이션 에디터는 혼합형이기 때문에 가져오는거 자체가 무겁다.
ㄴ 스켈레톤 에디터 / 스켈레톤 메시 에디터 / 애니메이션 에디터 / 애니메이션 블루프린트 에디터 / 피직스 에디터
(여기서 최적화 할 수 있다, 코드에서 작업하는게 편리하긴 하다.)

▶ 3D 애니메이션 선행 지식

- 이 개념을 2D로 바뀐게 스파인 / 라이브 2D 
ㄴ 스켈레탈 애니메이션 프로그램
ㄴ 관절 기반 -> 생동감 있는 애니메이션 동작이 가능하다.

- 애니메이션 -> 메시 + 스켈레톤 + (본) -> 본 안에 소켓이 들어간다.
...
- 3D 에서 기본적으로 사용하는 구조가 위에 구조 (FBX)

- 스켈레톤 -> 뼈대 -> 손끝부터 -> 어깨까지 -> 이어지는 부분이 되겠다.
 ㄴ 스켈레톤 -> IK에 속해 있는 개념
  ★ IK < = > FK

 ㆍ IK vs FK

 - 애니메이션 구조는 캐릭터의 구조상 대부분 트리 형식으로 구현이 된다.
  ㄴ 부모 < - > 자식

 - 메시 + 스켈 + 본으로 이루어진 전체를 -> IK 라고 부르기도 한다.
...
- 애니메이션은 결국 본이 중요하다.
ㄴ 스켈 -> 가상의 개념에 가깝고 본은 실제하는 트랜스폼을 가지고 있는 객체
ㄴ 중점이 될 수 있다. -> 이러한 중점이 여러개 모여 메시 구조를 이룬다.
ㄴ 형태를 유지 시켜준다.

B ======= B
     S
※ 결국 본과 본의 사이가 스켈레톤이 되는 것이다.

ㆍ모델 / 애니메이션 구조
- 보통 메시를 불러오면 스켈레톤도 함께 로드가 된다.
 ㄴ 정확히는 본 -> 스켈레톤 -> 메시

- 이런 개념때문에 리타겟팅은 결국 메시 기반이라고 하기 보다는
 본의 구조가 일치해야지만 쓸 수 있는 형태를 띄고 있다.
...
A
 ㄴ M
 ㄴ S - "IK (FK)"
  ㄴ B - "Key" (키프레임은 이미 연산된 부분을 저장해서 쓰기 때문에 연산량에 최적화에 좋다)
모바일에서 발열이 올라가는 이유는 적이 많아질 때 키 프레임을 그만큼 많이 불러오기 때문이다.


- 소켓
ㄴ 뭔가를 붙이기 위해서 마련해 놓는 언리얼만의 개념 (Bone)
ㄴ 유니티 같은 경우에는 기본적으로 계층 구조를 이용해서 처리하는 경우가 많다.
ㄴ 소켓은 본에 임의로 만들어 줄 수 있고 소켓이 있어야 다른 모델을 점착시킬 수 있는 구조
Animation
 ㄴ M
 ㄴ S - "IK (FK)"
  ㄴ B - "Key" , "Soket"
...
- 루트 모션 / 루트 본
 ㄴ 좌표 이동 + 역동적 + 물리적인 움직임을 처리하기 위해 나온 기법
  ㄴ 최상위 루트 노드의 위치 변화량을 기준으로 캐릭터의 월드 좌표에 반영하는 식
   ㄴ 스켈레톤 -> 루트 -> 본 -> 기준 -> 애니메이션 -> 캐릭터가 이 루트를 기준으로 해서
      동작하는 애니메이션을 루트 모션 기반 애니메이션이라고 부른다.
 ㄴ 캐릭터 움직임 -> 좌표 + 회전값 -> 원점 -> 루트 -> 반영 (후 처리가 많이 되어야 됨)
